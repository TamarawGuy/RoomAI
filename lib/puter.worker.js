...
router.post("/api/projects/save", async ({ req, user }) => {
    try {
        ...
        return jsonError(500, "Failed to save object", {
...
router.get("/api/projects/list", async ({ user }) => {
    try {
        const userPuter = user.puter;
        if (!userPuter) return jsonError(401, "Authentication failed: no user");

        const userId = await getUserId(userPuter);
        if (!userId) return jsonError(401, "Authentication failed: no user id");
...
router.get("/api/projects/get", async ({ req, user }) => {
    try {
        ...
        if (!project) return jsonError(404, "Porject not found");
